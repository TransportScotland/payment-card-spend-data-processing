...
pd.set_option('display.max_columns', 20) # for displaying more than 5 columns

def prepare_postcode_info_df(postcode_fpath) -> pd.DataFrame:
    """
    Read in file at postcode_fpath and combine unit rows into sector rows (geo locations averaged)
    """
    ...
    # extract only live and small-user postcodes (large-user are things like delivery centres)
...
def prepare_scotland_census_df(census_fpath):
    ...
    # Scottish census has rows like "AB10 3 (part something)" for postcodes that don't match up with census output areas 
...
def prepare_eng_wal_census_df(census_fpaths):
    ...
    # add missing spaces (change strings like AB102 to AB10 2)
    dfc.loc[dfc['sector'].str[3] != ' ', 'sector'] = dfc['sector'].str[:4] + ' ' + dfc['sector'].str[-1]
    # replace multiple spaces with one space (change strings like G1  1 to G1 1)
    dfc['sector'] = dfc['sector'].str.replace('  ', ' ', regex=False) 
    return dfc

def assign_proportional_numbers_ni(df1: pd.DataFrame, df1_end :pd.DataFrame):
    """NI census provides a count of hidden population numbers, distributing it evenly among postcode units here"""

    # extract the numbers from the special last few rows of the csv
...
def load_regions_file():
    """
    Load regions file (csv w rows like postcode_area,town_name,region_name)
    returns {Area: (Postal town, Region)}
    """
...
def collect_locations(dfm: pd.DataFrame):
    ...
    def drill_up(df: pd.DataFrame, col_name, location_level, 
            ...
            cols_first = None):
        """
        Combine the values for e.g. sector up to district.
        col_name: column to group by, location_level: name like it appears in our card data
        cols_sum, cols_mean, cols_first: columns to sum, get the mean of, or first occurrence of, respectively
        """
...
def add_parents_where_none(df: pd.DataFrame):
    """Add district and area information if it is missing and sector is available"""

    # this was meant to only happen for rows where district and area are null, but pandas makes that way too difficult
    df['district'] = df['sector'].str.extract(r'\b([A-Z]{1,2}\d{1,2}[A-Z]?)')
...
def etl(postcode_fpath, census_scotland_fpath, census_eng_wal_fpaths, census_ni_fpaths, 
    ...
    """
    Load in census and location information from csv files into a dataframe and saves into a json dict
    """
    ...
    dfm = dd.merge(dfp, dfc, left_on='sector', right_on='sector', how='outer') # could use pd.merge here but I don't want to break it
